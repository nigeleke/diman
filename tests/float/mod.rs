...
macro_rules! gen_tests_for_float {
    ($float_type: ty, $mod_name: ident, $assert_is_close: path, $assert_is_close_float: path) => {
        mod $mod_name {
            ...
            make_annotated_unit_constructor!(kilograms, Mass<$float_type>, $float_type);
            make_annotated_unit_constructor!(
                dimensionless,
                Dimensionless<$float_type>,
                $float_type
            );
            make_annotated_unit_constructor!(meters_per_second, Velocity<$float_type>, $float_type);
            make_annotated_unit_constructor!(newtons, Force<$float_type>, $float_type);
            make_annotated_unit_constructor!(joules, Energy<$float_type>, $float_type);

            #[test]
            fn add_same_unit() {
                let x = meters(1.0.into());
                let y = meters(10.0.into());
                assert_is_close(x + y, meters(11.0.into()));
            }
            ...
            // #[test]
            // fn constant() {
            //     assert_is_close((1.0 as $float_type) * SOLAR_MASS, kilograms(1.988477e30));
            //     assert_is_close(
            //         (1.0 as $float_type) * SOLAR_MASS_GRAMS,
            //         kilograms(1.988477e30),
            //     );
            //     assert_is_close(
            //         (1.0 as $float_type) * SOLAR_MASS_AWKWARD,
            //         kilograms(1.988477e30),
            //     );
            // }
